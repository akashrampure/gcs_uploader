...
func TestGCSUploaderIntegration(t *testing.T) {
	...
	expectedDownloadedPath := filepath.Join(downloadDir, filepath.Base(objectName))
	...
	t.Run("DownloadFile", func(t *testing.T) {
		n, err := uploader.DownloadFile(ctx, objectName, downloadDir)
		if err != nil {
		...
		data, err := os.ReadFile(expectedDownloadedPath)
		if err != nil {
	...
	t.Run("UploadFile", func(t *testing.T) {
		tmpFile := filepath.Join(t.TempDir(), "gcs_test_upload.txt")
